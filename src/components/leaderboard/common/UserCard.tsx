...
const UserCard = (props: UserCardProps) => {
    ...
    return (
        <li className='flex w-full grow items-center rounded-md bg-bgPurplePower/10 px-4 py-2 backdrop-blur-xl transition-all delay-75 duration-200 hover:bg-bgPurple/20'>
            <span className='inline-flex min-w-[2rem] max-w-[2rem] items-center justify-center font-semibold italic sml:text-lg mdl:text-xl'>{rank}</span>

            <div className='relative mx-3 h-11 w-11 rounded-full lgl:h-14 lgl:w-14'>
                <Image src={user.avatar ? user.avatar : '/assets/images/default_avatar.png'} alt='' fill className='rounded-full object-cover object-center' />
            </div>
